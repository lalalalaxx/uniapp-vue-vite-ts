...
const leftImgUrl = computed(() => {
    if (props.isBlackIcon) {
        return '/src/static/pubImgs/back.png'
    } else {
        return '/src/static/pubImgs/backW.png'
    }
})
...
<template>
    <view class="header">
        <view class="header_main" :style="headerStyle">
            <view class="status_bar" :style="{ height: statusBarHeight + 'px' }"></view>
            <!-- 标准的左中右结构 -->
            <!-- #ifndef MP-WEIXIN -->
            <view class="header flex-center-between" :style="{ height: headerHeightRef + 'px' }">
                <view class="header_left flex-between">
                    <!-- <view class="icon flex" @click="goback" v-if="leftIconShow">
                        <image :src="leftImgUrl" mode=" widthFix" />
                    </view> -->
                    <view class="left_slot" @click="goback" v-if="leftIconShow">
                        <slot name="left">
                            <image :src="leftImgUrl" mode=" widthFix" />
                        </slot>
                    </view>
            ...
            <view class="wx_header flex AI-center" :style="{ height: headerHeightRef + 'px', width: wxHeaderWidth + 'px' }">
                <view class="wx_header_left flex AI-center">
                    <view class="icon flex AI-center" @click="goback" v-if="leftIconShow">
                        <image :src="leftImgUrl" mode="widthFix"></image>
                    </view>
                    <view>
                        <slot name="left"></slot>
                    </view>
